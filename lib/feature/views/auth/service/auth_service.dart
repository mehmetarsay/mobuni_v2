...
class AuthService {
  ...
  void saveToken(String? token, UserModel user) {
    _hiveService!.hive.put(Constants.authToken, token);
    var a = locator<AuthService>().isLogin;
    print(a);
    _networkManager!.setHeaderToken(token!);
    _hiveService!.hive.put(Constants.user, user);
    UserModel userq = _hiveService!.hive.get(Constants.user);
  ...
  Future getUserById(String id) async {
    var users = _hiveService!.hive.get(Constants.users);
    users ??=  <String, UserModel>{};
    if ((users as Map<String, UserModel>).containsKey(id)) {
      return users[id];
    } else {
      var user = await _networkManager!.request(
          method: ReqTypes.get,
          path: ApiConstants.userById,
          model: UserModel(),
          queryParameters: {'UserId': id});
      users.addAll({id: user});
      _hiveService!.hive.put(Constants.users, users);
      return user;
    }
  }
}
