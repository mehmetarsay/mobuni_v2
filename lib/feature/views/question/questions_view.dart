...
class QuestionsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<QuestionsViewModel>.reactive(
      ...
      builder: (context, vm, child) => vm.initialised? Scaffold(
        appBar: CustomAppBar(
          actions: [
            Visibility(
              visible: true,
              child: IconButton(
                  ...
                  onPressed: () async {
                    ...
                    GeneralManager.navigationS.pushNamedAndRemoveUntil(Routes.loginView);
                    await OneSignal.shared.setExternalUserId('');
                  }),
            ),
            IconButton(
              icon: Icon(
                context.watch<ThemeNotifier>().darkTheme! ? Icons.light_mode : Icons.dark_mode,
              ),
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return QuestionSingleView(
                questionModel: box.get(HiveBoxKey.questions.name)[index],
                onTap: () async {
                  return await context.navigationService
                      .navigateToView(
                    CommentView(
